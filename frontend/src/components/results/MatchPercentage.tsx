...
export const MatchPercentage = ({ percentage }: MatchPercentageProps) => {
  ...
  return (
    <div className={`p-6 rounded-lg border-2 ${getBackgroundColor(percentage)} ${getBorderColor(percentage)}`}>
      <div className="flex items-center justify-center space-x-6">
        ...
        <div>
          <h3 className="text-xl font-semibold text-gray-800 mb-2">Resume Match Score</h3>
          <p className="text-sm text-gray-600">
            {percentage >= 80 && "Excellent match! Your resume aligns well with the job requirements."}
            {percentage >= 60 && percentage < 80 && "Good match! Consider incorporating more relevant keywords."}
            {percentage < 60 && "Needs improvement. Focus on highlighting relevant skills and experience."}
          </p>
        </div>
      </div>
    </div>
